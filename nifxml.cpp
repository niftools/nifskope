...
void NifModel::insertNiBlock( const QString & identifier, int )
{
	NifBlock * block = blocks.value( identifier );
	if ( block )
	{
		QModelIndex idy = insertBranch( NifData( identifier, "NiBlock" ), QModelIndex(), -1 );
...
void NifModel::inherit( const QString & identifier, const QModelIndex & idx, int )
{
	NifBlock * ancestor = ancestors.value( identifier );
	if ( ancestor )
	{
		QModelIndex idy = insertBranch( NifData( identifier ), idx, -1 );
...
void NifModel::insertType( const NifData & data, const QModelIndex & idx, int )
{
	...
	{	// recursive array construction
		...
		if ( evalCondition( idy ) )
		{
			...
			int dim = 0;
			QModelIndex dim1idx = getIndex( idx, d.arr1 );
			if ( ! dim1idx.isValid() || rowCount( dim1idx ) == 0 )
				dim = getInt( idx, d.arr1 );
			else
			{
				// extract array index from name
				int x = d.name.indexOf( '[' );
				int y = d.name.indexOf( ']' );
				if ( x >= 0 && y >= 0 && x < y )
				{
					//qDebug() << "extracted array index is " << name.mid( x+1, y-x-1 );
					int z = d.name.mid( x+1, y-x-1 ).toInt();
					dim = itemValue( index( z, 0, dim1idx ) ).toInt();
				}
				else
				{
					qCritical() << "failed to get array size for array " << d.name;
				}
			}
			QString name = d.name;
			
			if ( ! d.arg.isEmpty()  )	d.arg = QString( "(%1)" ).arg( d.arg );
			if ( ! d.arr2.isEmpty() )	d.arr1 = QString( "(%1)" ).arg( d.arr2 ); else d.arr1 = QString();
			d.arr2 = QString();
			d.cond = QString();
			d.ver1 = d.ver2 = 0;
